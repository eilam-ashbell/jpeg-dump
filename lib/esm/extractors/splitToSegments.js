...
function splitToSegments(rawFileData) {
    try {
        ...
        while (currentOffset < rawFileData.length - 1) {
            ...
            if (rawFileData[currentOffset] === 0xff) {
                ...
                if (rawFileData[currentOffset] !== 0x00) {
                    ...
                    if (!checkIfKnownMarker(markerKey))
                        continue;
                    // if marker have no length
                    if (Object.values(markersDict)
                        ...
                        fileStructure.push(new SegmentModel(rawFileData.slice(markerStart, currentOffset + 1), markerStart, fileStructure.length));
                    }
                    else {
                        ...
                        // create new segment model
                        fileStructure.push(new SegmentModel(rawFileData.slice(markerStart, markerStart + length + 2), markerStart, fileStructure.length));
        ...
        (lastMarker === null || lastMarker === void 0 ? void 0 : lastMarker.name) === "EOI" &&
            ...
            // create new segment model
            fileStructure.push(new SegmentModel(trailerData, lastMarker.globalOffset + 2, fileStructure.length));
        }
