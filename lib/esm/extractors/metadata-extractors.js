import { extractFirstIFDOffset, extractIFD, hexToReadable, ifdDataToTags, } from "../utils/exif-utils";
...
function extractExifTags(app1Segment) {
    ...
    for (let ifdKey of ifdKeys) {
        ...
        for (let tag in segmentTags[ifdKey]) {
            const extendedTag = new ExifExtendedTagModel(segmentTags[ifdKey][tag], "main");
            if (extendedTag.isValueAtOffset) {
                ...
                // parse the new value
                extendedTag.parsedValue = hexToReadable(extendedTag.tagId, extendedTag.dataTypeAsInt, extendedTag.valueCount, extendedTag.rawValue);
            }
...
function TIFFParser(app1Segment) {
    ...
    while (nextIFDOffset) {
        // get IFD data
        const IFDData = extractIFD(app1Segment, tiffHeaderOffset + nextIFDOffset); // (tiffHeaderOffset + nextIFDOffset) offset from start of segment
        ...
        if (tagsInIFD["8769"]) {
            ...
            const exifIFDData = extractIFD(app1Segment, tiffHeaderOffset + exifOffset); // (tiffHeaderOffset + exifOffset) offset to exifSubIFD
        ...
        if (tagsInIFD["8825"]) {
            ...
            const exifIFDData = extractIFD(app1Segment, tiffHeaderOffset + GPSOffset); // (tiffHeaderOffset + exifOffset) offset to exifSubIFD
        ...
        if (tagsInIFD["83bb"]) {
            if (tagsInIFD["83bb"]) {
                ...
                const IPTCData = extractIFD(app1Segment, tiffHeaderOffset + IPTCOffset); // (tiffHeaderOffset + IPTCOffset) offset to IPTC
        ...
        if (tagsInIFD["02bc"]) {
            ...
            const XMPData = extractIFD(app1Segment, tiffHeaderOffset + XMPOffset); // (tiffHeaderOffset + XMPOffset) offset to XMP
        ...
        if (tagsInIFD["014a"]) {
            ...
            const subIFDData = extractIFD(app1Segment, tiffHeaderOffset + subIFDOffset); // (tiffHeaderOffset + subIFDOffset) offset to exifSubIFD
        ...
        if (tagsInIFD["0190"]) {
            ...
            const subIFDData = extractIFD(app1Segment, tiffHeaderOffset + subIFDOffset); // (tiffHeaderOffset + subIFDOffset) offset to subIFDOffset
            const tagsInSubIFD = ifdDataToTags(subIFDData);
            // extend data of every tag in IFD
            for (const tag in tagsInSubIFD) {
                tagsInSubIFD[tag] = tagExtender(tagsInSubIFD[tag], app1Segment, "main");
        ...
        if (tagsInIFD["4748"]) {
            ...
            const microsoftStitchData = extractIFD(app1Segment, tiffHeaderOffset + microsoftStitchOffset); // (tiffHeaderOffset + microsoftStitchOffset) offset to microsoftStitch
        ...
        if (tagsInIFD["8290"]) {
            ...
            const kodakIFDData = extractIFD(app1Segment, tiffHeaderOffset + kodakIFDOffset); // (tiffHeaderOffset + kodakIFDOffset) offset to KodakIFD
