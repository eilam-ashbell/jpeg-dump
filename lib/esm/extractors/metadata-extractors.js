...
function TIFFParser(app1Segment) {
    ...
    while (nextIFDOffset) {
        // get IFD data
        const IFDData = extractIFD(APP1RawData, tiffHeaderOffset + nextIFDOffset); // (tiffHeaderOffset + nextIFDOffset) offset from start of segment
        ...
        if (tagsInIFD["8769"]) {
            ...
            const EXIFIFDData = extractIFD(APP1RawData, tiffHeaderOffset + EXIFOffset); // (tiffHeaderOffset + EXIFOffset) offset to EXIFSubIFD
        ...
        if (tagsInIFD["8825"]) {
            ...
            const EXIFIFDData = extractIFD(APP1RawData, tiffHeaderOffset + GPSOffset); // (tiffHeaderOffset + EXIFOffset) offset to EXIFSubIFD
        ...
        if (tagsInIFD["83bb"]) {
            if (tagsInIFD["83bb"]) {
                ...
                const IPTCData = extractIFD(APP1RawData, tiffHeaderOffset + IPTCOffset); // (tiffHeaderOffset + IPTCOffset) offset to IPTC
        ...
        if (tagsInIFD["02bc"]) {
            ...
            const XMPData = extractIFD(APP1RawData, tiffHeaderOffset + XMPOffset); // (tiffHeaderOffset + XMPOffset) offset to XMP
        ...
        if (tagsInIFD["014a"]) {
            ...
            const subIFDData = extractIFD(APP1RawData, tiffHeaderOffset + subIFDOffset); // (tiffHeaderOffset + subIFDOffset) offset to EXIFSubIFD
        ...
        if (tagsInIFD["0190"]) {
            ...
            const subIFDData = extractIFD(APP1RawData, tiffHeaderOffset + subIFDOffset); // (tiffHeaderOffset + subIFDOffset) offset to subIFDOffset
            const tagsInSubIFD = ifdDataToTags(subIFDData, app1Segment.globalOffset);
            // extend data of every tag in IFD
            for (const tag in tagsInSubIFD) {
                tagsInSubIFD[tag] = tagExtender(tagsInSubIFD[tag], APP1RawData, "main");
        ...
        if (tagsInIFD["4748"]) {
            ...
            const microsoftStitchData = extractIFD(APP1RawData, tiffHeaderOffset + microsoftStitchOffset); // (tiffHeaderOffset + microsoftStitchOffset) offset to microsoftStitch
        ...
        if (tagsInIFD["8290"]) {
            ...
            const kodakIFDData = extractIFD(APP1RawData, tiffHeaderOffset + kodakIFDOffset); // (tiffHeaderOffset + kodakIFDOffset) offset to KodakIFD
