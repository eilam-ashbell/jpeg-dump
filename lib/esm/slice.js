...
export default function slice(fileStructure) {
    const slices = [];
        ...
        if (segment.name === "APP0") {
            ...
            for (let IFD in EXIF.parsedTags) {
                for (let tag in EXIF.parsedTags[IFD]) {
                    nested.push(new SliceModel([IFD, tag], EXIF.parsedTags[IFD][tag].globalOffset, 
                    // 12,
                    EXIF.parsedTags[IFD][tag].tagRawValue));
                }
            }
            slices.push(new SliceModel(["APP1"], segment.globalOffset, 
            // segment.length,
            segment.rawData, nested));
        }
        else
            slices.push(new SliceModel([segment.name], segment.globalOffset, 
            // segment.length,
            segment.rawData));
    });
    console.log(slices);
